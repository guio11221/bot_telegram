...
const _messageTypes = ['text', 'animation', 'audio', 'channel_chat_created', 'contact',
    ...
    'voice', 'video_chat_started', 'video_chat_ended', 'video_chat_participants_invited',
    'video_chat_scheduled', 'message_auto_delete_timer_changed', 'chat_invite_link',
...
_request(_path, options = {}) {
  ...
  // Merge custom request options if provided
  if (this.options.request) {
    Object.assign(options, this.options.request);
  }

  // Helper function to clean reply-related fields
  const fixReplyFields = (target) => {
    this._fixReplyMarkup(target);
    this._fixEntitiesField(target);
    this._fixReplyParameters(target);
  };

  // Fix form data if present
  if (options.form) {
    fixReplyFields(options.form);
  }

  // Fix query string (qs) data if present
  if (options.qs) {
    fixReplyFields(options.qs);
  }

  // Set default HTTP options
  options.method = 'POST';
  options.url = this._buildURL(_path);
  options.simple = false;  // Disable status code validation to handle errors manually
  options.resolveWithFullResponse = true;  // Get the full response for further validation
  options.forever = true;  // Enable persistent HTTP connections
  ...
  return request(options)
    .then(resp => {
      ...
      try {
        ...
        resp.body = data;  // Update the response object with parsed data
      } catch (err) {
        // Throw an error if parsing fails
        throw new errors.ParseError(`Error parsing response: ${resp.body}`, resp);
      }

      // Check if the API response was successful (data.ok should be true)
      if (data.ok) {
        return data.result;
      }
    ...
    .catch(error => {
      if (error.response) {
        // If there is an HTTP response, propagate the error
        throw error;
      }
      // If no response, throw a fatal error (e.g., network or server issue)
